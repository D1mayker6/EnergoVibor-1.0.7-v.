...
public class GrayPoleScript : MonoBehaviour
{
    
    [SerializeField] private GameObject[] players=new GameObject[6];
    [SerializeField] private map map;
    [SerializeField] private Light2D GrayLight;
    [SerializeField] private BoxCollider2D ColliderGrayCard;
    ...
    private void Start()
    {
        audioSource = ColliderGrayCard.gameObject.GetComponent<AudioSource>();
        ...
        AnimCard = ColliderGrayCard.gameObject.GetComponent<Animator>();
    ...
    void Update()
    {
            ...
            && map.PlayerPositions[GameData.currentPlayer] == Convert.ToInt32(this.gameObject.name))
        {
            ...
            if (GameData.GrayCardOpenedCount[GameData.currentPlayer] < 1)
            {
                map.DicesLight.SetActive(false);
                GrayLight.enabled = true;
            }
            if (GameData.GrayCardOpenedCount[GameData.currentPlayer] < 1) mapp.enabled = false;
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                {
                    if (hit.collider.gameObject == ColliderGrayCard.gameObject)
                    {
                        if (GameData.GrayCardOpenedCount[GameData.currentPlayer] < 1)
                        {
    ...
    void GrayCard()
    {
        ...
        cam.enabled = true;
        mapp.enabled = true;
        SceneManager.LoadScene("GrayCard");
    }
    private IEnumerator GrayCardDrop()
    {
        yield return new WaitForSeconds(1f);
        GameData.GrayCardOpenedCount[GameData.currentPlayer]++;
